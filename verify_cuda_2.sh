...
for LIB_DIR in "/usr/lib/nvidia-cuda-toolkit/lib64" "/usr/lib/nvidia-cuda-toolkit/lib" "/usr/lib/x86_64-linux-gnu"; do
    if [ -d "$LIB_DIR" ] && ls $LIB_DIR/libcuda*.so* > /dev/null 2>&1; then
        LIB_COUNT=$(ls $LIB_DIR/libcuda*.so* 2>/dev/null | wc -l)
        print_success "CUDA toolkit libraries found: $LIB_COUNT libraries in $LIB_DIR/"
        CUDA_LIB_FOUND=1
        break
    fi
done

if [ $CUDA_LIB_FOUND -eq 0 ]; then
    print_failure "CUDA toolkit libraries not found in any standard location"
    echo "  Searched: /usr/lib/nvidia-cuda-toolkit/lib64, /usr/lib/nvidia-cuda-toolkit/lib, /usr/lib/x86_64-linux-gnu"
fi
echo ""
